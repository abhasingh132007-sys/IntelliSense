...
MODULES = [
    ...
    },
    {
        ...
        "sections": [
            ...
            {"type": "text", "heading": "The Incident Pipeline",
             "body": "Instead of a black box, modern IDPS platforms use a structured pipeline: 1) Detect: The engine "
                     "spots an anomaly and flags it as 'New'. 2) Escalate: A human Analyst reviews the packet capture, "
                     "adds context, and sends it up the chain. 3) Approve: A senior Administrator verifies the Analyst's "
                     "findings. 4) Block: The IPS drops the malicious IP. This ensures strict accountability at every step."},
    ...
    },
    {
        ...
        "sections": [
            ...
            {"type": "text", "heading": "The Netfilter Architecture",
             "body": "Linux handles networking inside the kernel via 'netfilter'. When a packet arrives at the network "
                     "interface, it hits the PREROUTING chain. If destined for the local machine, it moves to the INPUT chain "
                     "(where IntelliSense places blocks to protect the server). If it's leaving the machine, it uses the OUTPUT "
                     "chain. If being routed elsewhere, it passes through the FORWARD chain."},
